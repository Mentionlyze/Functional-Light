...
export function partialRight(fn: Function, ...presetArgs: any[]) {
...
const composeFn = compose(partialRight(reduce, sum, 0), partialRight(map, double), partialRight(filter, isOdd))
...
const curriedFilter = curry((predicateFn: (value: any, index: number) => value is any, arr: any[]) => arr.filter(predicateFn))

const curriedMap = curry((mappedFn: (value: any, index: number) => value is any, arr: any[]) => arr.map(mappedFn))
...
const composedCurriedFn = compose(curriedReduce(sum)(0), curriedMap(double), curriedFilter(isOdd))
...
const composedUnboundedFn = compose(unboundReduce(sum)(0), unboundMap(double), unboundFilter(isOdd))
